...
namespace Dancycodes\Gale\Services;
 ...
 *
 * @see \Dancycodes\Gale\Http\GaleSignal
 * @see \Dancycodes\Gale\Validation\GaleBase64Validator
 */
class GaleFileStorage
{
    /**
    ...
    public function storeMultiple(array $mapping, string $disk = 'public'): array
    {
        ...
        /** @var \Dancycodes\Gale\Http\GaleSignal $galeSignal */
        $galeSignal = signals();

        foreach ($mapping as $signalKey => $directory) {
            if ($galeSignal->has((string) $signalKey)) {
