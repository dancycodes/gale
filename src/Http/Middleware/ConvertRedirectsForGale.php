...
/**
 * Convert Redirects For Gale Middleware
 *
 * Automatically intercepts standard Laravel redirect responses in Gale requests and
 * converts them into Gale-compatible SSE responses with client-side JavaScript redirects.
 ...
 *
 * Standard Laravel redirects with Gale requests create a double-navigation scenario:
 ...
 *
 * The middleware registers globally via GaleServiceProvider, activating only for requests
 * with the Gale-Request header and skipping responses already processed by GaleRedirect.
 * No application-level configuration is required for operation.
 *
 * @see \Dancycodes\Gale\Http\GaleRedirect
 * @see \Dancycodes\Gale\GaleServiceProvider
 */
class ConvertRedirectsForGale
{
    /**
     * Handle an incoming request
     *
     * Processes responses from the application pipeline, intercepting redirect responses
     * for Gale requests and converting them to SSE responses with JavaScript navigation.
     * Non-Gale requests, non-redirect responses, and already-processed Gale responses
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // Pass through non-Gale requests unchanged
        if (!$request->hasHeader('Gale-Request')) {
            return $response;
        }

        // Skip responses already processed by GaleRedirect to prevent double-conversion
        ...
        // Convert redirect to Gale SSE response with JavaScript navigation
        return $this->convertToGaleRedirect($response, $request);
    }
    ...
    /**
     * Convert redirect response to Gale SSE response with JavaScript navigation
     *
     ...
     * GaleRedirect timing) to allow session persistence operations to complete before
     ...
     */
    protected function convertToGaleRedirect(Response $response, Request $request): \Symfony\Component\HttpFoundation\StreamedResponse
    {
        $redirectUrl = $this->getRedirectUrl($response);

        // Build SSE response with JavaScript navigation script
        $galeResponse = app('gale.response');
        ...
        /** @var \Symfony\Component\HttpFoundation\StreamedResponse $galeStreamedResponse */
        $galeStreamedResponse = $galeResponse
            ->js($script, ['autoRemove' => true])
            ->toResponse($request);

        // Preserve session flash data and attach session cookie to new response
        $this->preserveFlashData($galeStreamedResponse);

        // Transfer cookies from original response to maintain state (CSRF tokens, etc.)
        foreach ($response->headers->getCookies() as $cookie) {
            $galeStreamedResponse->headers->setCookie($cookie);
        }

        return $galeStreamedResponse;
    }
     ...
     * counteract the dual aging mechanism inherent in Gale redirect handling. When
