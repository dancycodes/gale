...
class GaleServiceProvider extends ServiceProvider
{
    ...
    private function registerBladeDirectives(): void
    {
        ...
        // Resolve manifest path at compile time so it's embedded as a literal string
        // in the compiled Blade PHP. __DIR__ inside heredoc would resolve to the Blade
        // cache directory at runtime, not the package's src/ directory.
        $manifestPath = str_replace('\\', '/', __DIR__ . '/../resources/gale-manifest.json');

        Blade::directive('gale', function (string $expression) use ($manifestPath) {
...
<?php
    ...
    // Cache busting: read content-hash manifest generated by build.js
    \$__galeManifestPath = '{$manifestPath}';
    \$__galeJsHash = '';
    \$__galeCssHash = '';
    if (file_exists(\$__galeManifestPath)) {
        \$__galeManifest = json_decode(file_get_contents(\$__galeManifestPath), true) ?: [];
        \$__galeJsHash = isset(\$__galeManifest['js']) ? '?id=' . \$__galeManifest['js'] : '';
        \$__galeCssHash = isset(\$__galeManifest['css']) ? '?id=' . \$__galeManifest['css'] : '';
    }
    echo '<meta name="csrf-token" content="' . csrf_token() . '">' . chr(10)
        ...
        . '<link rel="stylesheet" href="' . asset('vendor/gale/css/gale.css') . \$__galeCssHash . '">' . chr(10)
        . '<script type="module" src="' . asset('vendor/gale/js/gale.js') . \$__galeJsHash . '"' . \$__galeNonceAttr . '></script>';
    unset(\$__galeOptions, \$__galeNonce, \$__galeNonceAttr, \$__galeNonceScript, \$__galeDebugScript, \$__galeSanitizeHtml, \$__galeAllowScripts, \$__galeXssScript, \$__galeRedirectConfig, \$__galeRedirectScript, \$__galeManifestPath, \$__galeManifest, \$__galeJsHash, \$__galeCssHash);
?>
PHP;
        });
